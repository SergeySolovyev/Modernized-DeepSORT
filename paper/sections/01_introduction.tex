...
boxes are fixed.
\item A counterintuitive \textbf{``more detections hurt''} regime that the headline mean conceals: at its
default gate the modern tracker scores \emph{below} the 2017 baseline on the least-precise clip, and across
all \nVideos{} videos lowering the detector confidence gate never raises and usually lowers HOTA, with the
size of the penalty predicted by per-clip detector precision (Pearson $r = \corrPearson$).
