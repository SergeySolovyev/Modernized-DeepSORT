...
gate before reaching the tracker: a confidence threshold ($\text{min\_confidence}$), a
non-maximum-suppression (NMS) step on overlapping boxes, and a minimum box-height filter. The
...
spurious new tracks. The gate itself is still present in the code path (the same confidence filter
plus NMS that the original used), merely disabled by default, which makes the restoration ablation
below a one-line configuration change rather than a code change.
...
versus association. All trackers run in real time on this hardware (mean \fpsRealtime{}~FPS).
...
HOTA response with the detector's per-clip precision at conf~0.25 to test whether the \emph{sign} of
recall's effect on HOTA is predicted by per-clip precision.
...
additionally sweep the detector input resolution over $\{960, 1280, 1536\}$. Larger input recovers
more small instances -- an \emph{orthogonal} recall lever to the confidence threshold -- and lets us
check that the "more detections hurt'' effect is a property of recall itself, not of any one knob.

\paragraph{(d) Gating-restoration ablation.}
To attribute the regression specifically to the dropped gate, we restore the original DeepSORT
contract -- $\text{min\_confidence}=0.3$ together with NMS at overlap~0.7 -- on top of the otherwise
unchanged modern pipeline, and re-measure HOTA on each video. This is the same gate the original used,
re-enabled by configuration alone, so any recovered HOTA is causally attributable to the gate and not
to a confounding code change.
...
experimental history is traceable and the figures in \cref{sec:results} are regenerable from it. The
