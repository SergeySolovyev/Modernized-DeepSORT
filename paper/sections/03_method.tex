...
of recall's effect on HOTA is associated with per-clip precision.
...
(sha256 \texttt{5d4a90cd\dots4fe5}) at input size~1280. Per-clip detector precision depends on both the
input size and the weights, so we fix and report both: the precision values in \cref{tab:detector} and
the money plot are measured at input size~1280 with the pinned checkpoint, and are recorded in the
journal as the \texttt{detector\_imgsz1280} rows. At the report track's smaller default input size the
same detector is markedly more precise on TUD-Campus ($0.73$ versus $0.43$), itself an instance of the
resolution effect studied in \cref{sec:hurt}; we therefore state the analysis at a single pinned
configuration rather than asserting a configuration-independent precision.
...
precision, input-size, and ablation plots) are regenerable from it; the per-clip precision behind the
money plot is the \texttt{detector\_imgsz1280} journal rows, the ground-truth-box re-identification
scores (\osnetGT{}/\timmGT{}/\osnetainGT{}) are the six-clip means of the \texttt{gt\_*\_gtbox} HOTA
rows, and the detection/association decomposition of \cref{tab:decomp} is a separate TrackEval run on
the combined MOT16 split under the default configuration. The
