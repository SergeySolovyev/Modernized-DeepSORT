...
precision, input-size, and ablation plots) are regenerable from it; the ground-truth-box re-identification scores
(\osnetGT{}/\timmGT{}/\osnetainGT{}) are the six-clip means of the \texttt{gt\_*\_gtbox} HOTA rows
in that journal, while the detection/association decomposition of \cref{tab:decomp} is a separate
TrackEval run on the combined MOT16 split under the default configuration. The
