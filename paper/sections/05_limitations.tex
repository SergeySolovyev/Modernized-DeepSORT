...
tuning heuristic, not a tuned constant. This caveat extends to the headline
number. The gated threshold (conf $0.45$) of \cref{tab:fullpipeline} was chosen
by observing the confidence sweep on the same six sequences on which it is then
evaluated, with no held-out split, so the gated mean (\modernHOTA{},
$+\deltaHOTA$) and the per-clip best (\modernBestHOTA{}) are in-sample estimates
and likely optimistic; $0.45$ is not even uniformly optimal (PETS09 peaks at conf
$0.25$). The 2017 baseline receives no equivalent per-clip tuning, so the
baseline-versus-gated comparison is one of unequal tuning; the selection-free
figure is the uniform-default modernization ($+\deltaDefaultHOTA$ at conf $0.25$).
...
\cite{zhou2019osnet,wightman2019timm} isolates association under
ground-truth boxes, where the gap is under one HOTA point; under real detections
that gap widens to \reidRealGap{} HOTA (\cref{sec:decomp}), and robustness across
