...
All numbers in this section are from the live Colab run of 2026-06-19 and are recorded in
\texttt{results/experiments.csv} and \texttt{report/results\_table.md}.
...
YOLOv8m has high mean recall ($0.831$) but its precision varies by sequence: it is high on the
cleaner videos and low on the dense low-resolution clips (precision $0.51$ on both TUD-Campus and
KITTI-17), where it produces many false-positive boxes. NanoDet and RTMDet were not installed on the
...
\midrule
YOLOv8m (box)            & 0.683 & 0.831 & 0.740 \\
YOLOv8m-seg (mask-to-box) & 0.696 & 0.831 & 0.749 \\
...
is marginally better than the box detector on detection F1 ($0.749$ vs $0.740$) and on mean
\hota{} ($51.86$ vs $51.45$), but the mask head roughly halves the throughput
($8.91$ vs $14.68$~FPS); both remain above the real-time threshold. The result is a quality and speed
trade-off rather than a clear improvement, so the box detector is the default and segmentation is a
switchable option (Figure~\ref{fig:seg}).
